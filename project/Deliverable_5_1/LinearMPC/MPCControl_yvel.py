...
class MPCControl_yvel(MPCControl_base):
    ...
    def compute_steady_state_with_disturbance(self, r: np.ndarray, d: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
        ...
        if prob.status not in [cp.OPTIMAL, cp.OPTIMAL_INACCURATE]:
            print(f"Warning: Y-vel steady-state infeasible. Status: {prob.status}, ||d||={np.linalg.norm(d):.3f}")
            ...
            if prob_fallback.status in [cp.OPTIMAL, cp.OPTIMAL_INACCURATE] and xss_var.value is not None:
                ...
                print(f"Warning: Y-vel fallback also failed, using nominal xs/us")
                xss = self.xs.copy()
                uss = self.us.copy()
        else:
            if xss_var.value is not None and uss_var.value is not None:
                ...
                print(f"Warning: Y-vel steady-state solver returned None, using nominal")
                xss = self.xs.copy()
                uss = self.us.copy()
