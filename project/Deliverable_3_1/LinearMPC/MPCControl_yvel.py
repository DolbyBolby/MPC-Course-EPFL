...
class MPCControl_yvel(MPCControl_base):
    ...
    def _setup_controller(self) -> None:
       ...
       #plot terminal set
        ...
        axes[0].set_title('O projected along wx and alpha')
        axes[0].set_xlabel('wx [rad/s]')
        axes[0].set_ylabel('alpha [rad]')
        axes[0].grid(True)
            # Projection sur les dimensions (1, 2)
        O.projection(dims=(1, 2)).plot(ax=axes[1], color='red')
        axes[1].set_title('O projected along alpha and vy')
        axes[1].set_xlabel('alpha [rad]')
        axes[1].set_ylabel('vy [m/s]')
        axes[1].grid(True)
            # Projection sur les dimensions (0, 2)
        O.projection(dims =(0,2)).plot(ax=axes[2], color='green')
        axes[2].set_title('O projected along wx and vy')
        axes[2].set_xlabel('wx [rad/s]')
        axes[2].set_ylabel('vy [m/s]')
        axes[2].grid(True)
        plt.suptitle('Maximal Invariant Set - yvel Controller', fontsize=14, fontweight='bold')
        plt.tight_layout()
        plt.show()
